...
public class GenreDetailActivity extends AppCompatActivity implements OnItemClickListener<Song>, SearchView.OnQueryTextListener {
    DatabaseHandler db;
    Context context;
    int offset = 0;
    int limit = 20;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String genreTemp = i.getStringExtra("genre");
        StringTokenizer tokenizer = new StringTokenizer(genreTemp, "_");
        int genreId = 0;
        String genreName = "";
        int totalSongs = 0;
        while (tokenizer.hasMoreTokens()) {
            genreId = Integer.parseInt(tokenizer.nextToken());
            genreName = tokenizer.nextToken();
            totalSongs = Integer.parseInt(tokenizer.nextToken());
        }
        id = genreId;
        getSupportActionBar().setTitle(genreName);
        getSupportActionBar().setSubtitle("Có " + totalSongs + " bài hát.");
        listSong = db.getSongsLimitOfGenre(genreId, limit, offset);

        System.out.println(listSong.size() + " " + genreId);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                /*Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        PlayMusicService.setMode(AppSetting.MODE_ONLINE);
                        PlayMusicService.addListAndClean((ArrayList) listSong);
                        PlayMusicService.addNewSongIndex(listSong.get(pos).getSongId());
                    }
                });
                thread.start();*/
            }
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                ...
                if (scrollState == SCROLL_STATE_IDLE) {
                    if (list.getLastVisiblePosition() >= count - threshold) {
                        ...
                        listSong.addAll(db.getSongsLimitOfGenre(id, limit, offset));
...
//                }
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        if (newText.length() > 0) {
            List<Song> songList = db.searchSongsOf(newText, id, 0);
            listSong.clear();
