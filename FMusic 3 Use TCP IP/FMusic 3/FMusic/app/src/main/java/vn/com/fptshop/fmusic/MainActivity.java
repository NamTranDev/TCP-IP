...
import vn.com.fptshop.fmusic.sync.SyncActivity;
...
//        }
        ...
        ArrayList<File> files = mediaManager.getAllFileMusicFromGalleries();
        for (int i=0;i<files.size();i++)
        {
            Log.d("FILE", files.get(i).getPath());
        }
    }
...
//        }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_song_new) {
            ...
            currentShow = 7;
            displayView(7);
        } else if (id == R.id.nav_sync_contacts) {
